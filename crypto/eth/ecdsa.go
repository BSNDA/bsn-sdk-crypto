...
func SignECDSA(privk *ecdsa.PrivateKey, digest []byte) (signature []byte, err error) {

	_, _, _, signature, err = SignData(privk, digest)
...
func VerifyECDSA(k *ecdsa.PublicKey, signature, digest []byte) (bool, error) {
	...
	v := crypto.VerifySignature(FromECDSAPub(k), digest, signature)
	return v, nil
}
...
func PubkeyToAdress(pub ecdsa.PublicKey) common.Address {
